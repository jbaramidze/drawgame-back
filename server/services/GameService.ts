...
export class GameService {
    ...
    constructor(private readonly helper: GameServiceHelpers, private readonly locker: BaseLocker, private readonly auth: AuthService) {}
    ...
    public async savePic(ctx: Context, code: string, user: string, pic: string): Promise<Response<null>> {
        return this.locker.withLock(ctx, code, async () => {
            ...
            const updated = await Game.updateOne(
                {code, players: {$elemMatch: {name: user}}},
                {
                    $set: {
                        "players.$.pic": pic,
    ...
    public async pickWord(ctx: Context, code: string, player: string, word: string): Promise<Response<null>> {
        return this.locker.withLock(ctx, code, async () => {
            ...
            const updated = await Game.updateOne(
                {code, players: {$elemMatch: {name: player}}},
                {
                    $set: {
